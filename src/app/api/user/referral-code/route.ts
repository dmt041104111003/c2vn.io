...
export const GET = withAuth(async (req, currentUser) => {
  try {
    ...
    return NextResponse.json(createSuccessResponse({
      referralCode,
      shareUrl: referralCode ? `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}?ref=${referralCode}` : null
    }));
  } catch (error) {
  ...
  try {
    ...
    if (action === 'generate') {
      ...
      return NextResponse.json(createSuccessResponse({
        ...
        shareUrl: `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}?ref=${newReferralCode}`
      }));
    }
